...
def _symbols_to_streams(layer_id: str, symbols: Any, meta: dict[str, Any]) -> list[SymbolStream]:
    ...
    if isinstance(symbols, list) and (len(symbols) == 0 or isinstance(symbols[0], int)):
        ...
        return [
            SymbolStream(
                name="main", kind="ids", alphabet_size=vocab_size, n=len(symbols), data=symbols
            )
        ]
    ...
    if isinstance(symbols, tuple) and all(isinstance(x, (bytes, bytearray)) for x in symbols):
        names = (
            ("mask", "vowels", "cons")
            if layer_id == "vc0" and len(symbols) == 3
            else tuple(f"s{i}" for i in range(len(symbols)))
        )
        out: list[SymbolStream] = []
        for name, part in zip(names, symbols, strict=False):
            pb = bytes(part)
...
def _decode_streams_with_optional_meta(
    layer_id: str, layer: Any, streams: list[SymbolStream]
) -> bytes:
    """
...
def encode_v5_payload(input_bytes: bytes, layer_id: str, layer: Any, codec: Any) -> bytes:
    ...
    if meta_bytes is not None:
        streams.append(
            SymbolStream(
                name="__meta__", kind="bytes", alphabet_size=256, n=len(meta_bytes), data=meta_bytes
            )
        )
    ...
    if codec_id == "zstd":
        ...
        if len(streams) == 1 and streams[0].name == "main" and streams[0].kind == "bytes":
...
def decode_v5_payload(
    payload: bytes, container_meta: dict[str, Any], layer_id: str, layer: Any, codec: Any
) -> bytes:
    """
    payload -> bundle (Huffman o Zstd) OR legacy v5 -> layer.decode -> raw bytes
    """
    getattr(codec, "codec_id", "huffman")
