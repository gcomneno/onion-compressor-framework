...
# -------------------
def pack_huffman_payload_ids(vocab_size: int, freq: List[int], lastbits: int, bitstream: bytes) -> bytes:
...
def unpack_huffman_payload_ids_inline_vocab(payload: bytes) -> Tuple[List[bytes], List[int], int, bytes]:
    if len(payload) < 1 + 4 + 4 + 1:
